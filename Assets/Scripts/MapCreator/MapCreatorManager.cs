	...
	void generateBlankMap(int mSize) {
		...
		for (int i = 0; i < mapSize; i++) {
			...
			for (int j = 0; j < mapSize; j++) {
					float PlotX = (float)i;
					float PlotY = (float)j;
				Tile tile = ((GameObject)Instantiate(PrefabHolder.instance.BASE_TILE_PREFAB, new Vector3 (PlotX * 2, (PlotY + ((PlotX % 2) / 2)) * 2, 0f) , Quaternion.Euler(new Vector3()))).GetComponent<Tile>();
